    ...
    F: FnOnce(u32) -> Option<&'a Label> + Copy,
{
    ...
    match pair.paired {
        Paired::OnlyLabel(l) => {
            write_glabel(f, l)?;
            write_incbin(f, info, rom_offset, pair.size)?;
        }
        Paired::OnlyData(d) => {
            write_parsed_data(f, d, find_label, pair.size, rom_offset, info)?;
        }
        Paired::Both(l, d) => {
            write_glabel(f, l)?;
            write_parsed_data(f, d, find_label, pair.size, rom_offset, info)?;
    ...
    F: FnOnce(u32) -> Option<&'a Label> + Copy,
{
    ...
    match (overflow, is_asciz(data)) {
        ...
        (true, true) => {
            writeln!(f, "# ERROR BAD ASCII @ {:#08X}", data.addr)?;
            write_incbin(f, info, rom_offset, total_size)?;
            return Ok(());
        }
    };

    write_data_entry(f, fl, data)?;
    if bytes_left > 0 {
        write_incbin(f, info, rom_offset, bytes_left)?;
    }
    Ok(())
}
...
fn write_data_entry<'a, F>(f: &mut Wtr, find_label: F, entry: &DataEntry) -> io::Result<()>
where
    F: FnOnce(u32) -> Option<&'a Label> + Copy,
{
    use ParsedData::*;
    ...
    writeln!(f, " # @ {:08X}", entry.addr)?;

    if is_asciz(entry) {
        writeln!(f, "{:4}.balign 4", "")?;
    }
